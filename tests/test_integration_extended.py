...
class TestSessionObject:
    ...
    def test_session_creation(self):
        ...
        assert session.status == SimulationStatus.PENDING
    ...
    def test_session_with_findings(self):
        ...
        session.red_findings.append(Finding(title="Test", severity="High", description="Test"))
        session.blue_findings.append(Finding(title="Test", severity="Medium", description="Test"))
...
class TestConcurrency:
    ...
    async def test_concurrent_orchestrator_sessions(self, mock_llm_engine, mock_vector_store):
        ...
        sessions = [SimulationSession(id=f"session-{i}", target=f"target-{i}") for i in range(10)]
...
class TestPerformance:
    ...
    async def test_session_creation_performance(self):
        ...
        sessions = [SimulationSession(id=f"session-{i}", target=f"target-{i}") for i in range(100)]
