...
# Unset conflicting environment variables before any imports
# Some systems (like Kali Linux) have AGENT=1 set which conflicts with Pydantic Settings
...
class TestExpiredTokenRejection:
    ...
    def test_token_expiry_time(self, security_module):
        ...
        data = {"sub": "test_user"}
        token = security_module.create_access_token(data, expires_delta=timedelta(seconds=1))
...
class TestSSRFValidation:
    ...
    def test_block_cloud_metadata_endpoints(self):
        ...
        is_valid, error = validate_url_for_ssrf(
            "http://169.254.169.254/latest/meta-data/", allow_localhost=False
        )
        assert is_valid is False
        ...
        is_valid, error = validate_url_for_ssrf(
            "http://169.254.169.254/metadata/v1/", allow_localhost=False
        )
        assert is_valid is False
...
class TestAuthManager:
    ...
    def test_register_agent(self, auth_manager):
        ...
        api_key = auth_manager.register_agent(
            agent_id="red-agent-1", agent_type="red", roles=["agent", "red"]
        )
    ...
    def test_sign_and_verify_request(self, auth_manager):
        ...
        is_valid = auth_manager.verify_signature(
            api_key, timestamp, signature, '{"action": "attack"}'
        )
        assert is_valid is False
