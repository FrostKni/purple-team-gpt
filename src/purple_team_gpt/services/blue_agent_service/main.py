"""Blue Agent Service - Standalone service for defensive security operations.

This service runs on a dedicated defender system with access to
defensive security tools and monitoring capabilities.
"""
...
def get_auth_manager() -> AuthManager:
    ...
    if auth_manager is None:
        import os
        secret = os.getenv("SECRET_KEY", "blue-agent-secret-key")
...
async def lifespan(app: FastAPI):
    ...
    # Initialize
    logger.info("Starting Blue Agent Service...")
    ...
    logger.info("Blue Agent Service ready")
    
    yield
    
    # Cleanup
    logger.info("Shutting down Blue Agent Service...")
    event_task.cancel()
    try:
        await event_task
...
async def register_with_orchestrator():
    ...
    agent_id = os.getenv("AGENT_ID", "blue-agent-1")
...
# Create FastAPI app
app = FastAPI(
    title="Blue Agent Service",
    description="Distributed defensive security agent",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def get_status():
    ...
    return {
        ...
        "monitored_system": agent_state.monitored_system,
        "steps_count": len(agent_state.steps),
        "findings_count": len(agent_state.findings),
        "detections_count": len(agent_state.detections),
        "defense_actions_count": len(agent_state.defense_actions),
...
async def receive_command(
    ...
    try:
        ...
        elif command.command_type == CommandType.RESPOND_EVENT:
            return await handle_respond_event(command, state)
...
async def handle_initialize(command: CommandMessage, state: AgentState):
    ...
    state.monitored_system = payload.get("target", "")
    state.scope = payload.get("scope", "")
    state.session_id = command.session_id
    state.steps = []
    state.findings = []
    state.detections = []
    state.defense_actions = []
    state.status = AgentStatus.INITIALIZING
    
    # Initialize conversation
    state.conversation = Conversation()
    state.conversation.system_prompt = get_blue_agent_prompt()
    ...
    return {
        "success": True,
        "message": f"Monitoring system: {state.monitored_system}",
    }


async def handle_respond_event(command: CommandMessage, state: AgentState):
    """Handle respond_event command - respond to Red agent activity."""
    ...
    context = f"""Event received from {source}:
Type: {event_type}
Severity: {severity}
Data: {data}

Analyze this event and determine appropriate defensive actions.
Consider:
1. Is this a threat indicator?
2. What immediate response is needed?
3. What containment measures should be implemented?
4. What hardening recommendations apply?
"""
    
    state.conversation.add_user(context)
...
async def receive_event(
    ...
    """Receive an event from Red agent (via orchestrator)."""
    logger.info(f"Received event from {event.agent}: {event.event_type}")
    ...
    # If it's a finding from Red agent, analyze it
    if event.event_type == EventType.FINDING:
        # Queue for processing
        # In a real implementation, this would trigger defensive actions
        pass
    
    return {"success": True, "detection_id": len(state.detections)}
...
async def execute_action(action: Dict[str, Any], state: AgentState) -> Dict[str, Any]:
    ...
    elif action_type in ("finding", "detection"):
        finding = {
            "title": action.get("title", "Unknown Detection"),
            "severity": action.get("severity", "Medium"),
            "description": action.get("description", ""),
            "evidence": action.get("evidence", ""),
            "recommendation": action.get("recommendation", action.get("response", "")),
        }
...
def get_blue_agent_prompt() -> str:
    """Get the system prompt for the Blue Agent."""
    return """You are the Blue Agent, an autonomous defensive security AI.

IDENTITY:
You are part of Purple Team GPT, a cybersecurity simulation framework.
Your role is to detect, respond to, and recover from security threats.

CAPABILITIES:
- Detection: Log analysis, anomaly detection, threat identification
- Response: Incident containment, firewall rules, service management
- Recovery: System restoration, rollback procedures
- Hardening: Security configurations, compliance checks

AVAILABLE TOOLS:
- log_analyzer: Analyze system logs
- firewall_manager: iptables/ufw management
- process_monitor: Monitor processes
- netstat_analyzer: Network analysis
...
- Port scanning: Sequential connections
...
```json
{
  ...
  "explanation": "Blocking malicious IP"
}
```
